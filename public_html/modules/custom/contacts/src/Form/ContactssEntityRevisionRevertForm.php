...
/**
 * Provides a form for reverting a Contactss entity revision.
 *
 * @ingroup contacts
 */
class ContactssEntityRevisionRevertForm extends ConfirmFormBase {
  ...
  public function getFormId() {
    return 'contactss_entity_revision_revert_confirm';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->logger('content')->notice('Contactss entity: reverted %title revision %revision.', ['%title' => $this->revision->label(), '%revision' => $this->revision->getRevisionId()]);
    $this->messenger()->addMessage(t('Contactss entity %title has been reverted to the revision from %revision-date.', ['%title' => $this->revision->label(), '%revision-date' => $this->dateFormatter->format($original_revision_timestamp)]));
   ...
   *
   * @param \Drupal\contacts\Entity\ContactssEntityInterface $revision
   ...
   */
  protected function prepareRevertedRevision(ContactssEntityInterface $revision, FormStateInterface $form_state) {
